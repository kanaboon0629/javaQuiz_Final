...
public class MergeServer {
  ...
  private List<DatagramPacket> packets = new CopyOnWriteArrayList<>();
  ...
  public void StartUDPChat() throws IOException {
    udpSocket = new DatagramSocket(SERVER_UDP_PORT);

    udpExecutorService = Executors.newSingleThreadExecutor();

    udpExecutorService.execute(
      () -> {
        while (true) {
          try {
            ...
            } else if (chatMessage.startsWith("udpmessage")) {
              chatMessage = chatMessage.substring(10);
              for (DatagramPacket sendpacket : packets) {
                byte[] sendData = chatMessage.getBytes(StandardCharsets.UTF_8); // Create new buffer
                ...
                packet =
                  new DatagramPacket(
                    ...
                    sendpacket.getPort() // Use the client's socket port
                  );
              ...
              if (loggedInUsers.contains(username)) {
                ...
                try {
                  ...
                  while ((message2 = in.readLine()) != null) {
                    System.out.println(
                      "message at ClientHandler = " + message2
                    );
          ...
          if (
            ...
            if (userExists(username)) {
              ...
              try (FileWriter writer = new FileWriter(USERS_FILE, true)) {
                //ユーザー情報を書き込む
                writer.write(
                  username + "," + password + "," + 0 + System.lineSeparator()
                );
  ...
  private static boolean verifyPassword(
    ...
    try (Stream<String> lines = Files.lines(Paths.get(USERS_FILE))) {
      return lines.anyMatch(
        line -> line.startsWith(loginusername + "," + loginpassword + ",")
      );
    } catch (IOException e) {
      return false;
    }
  ...
  private class Node {
    ...
    public void rightPlus() {
      ...
      if (this.samescore != null) {
        this.samescore.rank += 1;
        this.samescore.rightPlus();
      }
      if (this.left != null) {
        this.left.rank += 1;
        this.left.rightPlus();
    ...
    public boolean existSameScore(int num) {
      ...
      if (this.right != null && this.left != null) {
        return (
          this.right.existSameScore(num) || this.left.existSameScore(num)
        );
      } else if (this.right != null) return this.right.existSameScore(
          num
        ); else if (this.left != null) return this.left.existSameScore(
          num
        ); else if (this.score == num) return true; else return false;
    }
  ...
  public void add_leaf(Node n,boolean p){
    ...
    public void showInfo() {
      ...
      if (!(this.username == null)) System.out.println(
        this.rank + this.username + this.score
      );
  ...
  private class ClientHandler { //implements Runnable {
    ...
    public void yourRank(Node n, int num) {
      if (n.score == num) out.println(
        "TOTALSCORE_s " + "your rank is " + n.rank
      ); else {
    ...
    public void listUser(Node n) {
      ...
      if (!n.username.equals("head")) out.println(
        "TOTALSCORE_s " + n.rank + " : " + n.username + " " + n.score
      );
    ...
    public void winner(Node n) {
      if (n.username.equals("head") && n.left == null && n.samescore == null) {
        ...
        if (n.rank == 1) {
          ...
          if (!n.username.equals("head")) out.println(
            "TOTALSCORE_s " + "the winner is " + n.username
          );
        } else {
    ...
    private void handleMessage(String message) {
      ...
      if (message.startsWith("ANSWER_c ")) {
        ...
        for (ClientHandler client : server.clients) {
          this.head.add_leaf(
              new Node(client.name, client.score),
              head.existSameScore(client.score)
            );
        }
        ...
        try (
          ...
          while ((line = reader.readLine()) != null) {
            ...
            if (userparts.length >= 3 && userparts[0].equals(finalusername)) {
              ...
              //クライアントトータルスコアを伝える
              out.println(
                "TOTALSCORE_s " + "your total score is " + TotalScore
              );
    ...
    private void handleAnswer(String clientAnswer, String clientusername) {
      ...
      if (answer != null && clientAnswer.equalsIgnoreCase(answer)) {
        ...
        for (ClientHandler client : clients) {
          client.out.println(
            "ANSWER_s " + clientusername + " " + client.answer
          );
        }
